...
// 当前学校的所有商家获取某一天的销售应得收入数据
function create_excel($data = [],$name = 'excel')
{

    $excel = new \PHPExcel();
//     iconv('UTF-8', 'gb2312', $name); //针对中文名转码
    $header= ['序号','商家名称','下单金额','退款金额','实际收入']; //表头,名称可自定义
...
// 当前某个商家的某一天销售应得收入数据
function create_shop_excel($data = [],$name = 'excel')
{

    $excel = new \PHPExcel();
//     iconv('UTF-8', 'gb2312', $name); //针对中文名转码
    $header= ['序号','订单编号','备注信息','时间','金额']; //表头,名称可自定义
    $excel->setActiveSheetIndex(0);
    $excel->getActiveSheet()->setTitle($name); //设置表名
    $excel->getActiveSheet()->getDefaultRowDimension()->setRowHeight(18);
    $excel->getActiveSheet()->getColumnDimension('A')->setWidth(8);
    ...
    foreach($data as $k=>$v)
    {
        //从第二行开始写入数据（第一行为表头）
        $excel->getActiveSheet()->setCellValue('A'.($k+2),$k+1);
        $excel->getActiveSheet()->setCellValue('D'.($k+2),$v['add_time']);
        $excel->getActiveSheet()->setCellValue('B'.($k+2),$v['withdraw_sn']);
        $excel->getActiveSheet()->setCellValue('C'.($k+2),$v['title']);
        $excel->getActiveSheet()->setCellValue('E'.($k+2),$v['money']);
    }
